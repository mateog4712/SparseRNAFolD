...
typedef std::vector< cand_entry_t > cand_list_t;s
/**
...
public:
    ...
    */
     ...
     */
    const std::string &
    trace_back(bool mark_candidates=false) {
	mark_candidates_=mark_candidates;
	...
	if (mark_candidates_ && is_candidate(i,j)) {
	    structure_[i]='[';
	    structure_[j]=']';
	} else {
	    structure_[i]='(';
