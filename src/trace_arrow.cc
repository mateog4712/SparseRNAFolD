...
TraceArrows::TraceArrows(cand_pos_tu n) : n_(n), ta_count_(0), ta_avoid_(0), ta_erase_(0), ta_max_(0) {}

TraceArrow &trace_arrow_from(TraceArrows &t, cand_pos_tu i, cand_pos_tu j) { return t.trace_arrow_[i].find(j)->second; }

bool exists_trace_arrow_from(TraceArrows &t, cand_pos_tu i, cand_pos_tu j) { return t.trace_arrow_[i].exists(j); }
 ...
 * @param n New size
 */
void resize(TraceArrows &t, cand_pos_tu n) { t.trace_arrow_.resize(n); }
...
cand_pos_tu erasedT(TraceArrows &t) { return t.ta_erase_; }
cand_pos_tu sizeT(TraceArrows &t) { return t.ta_count_; }
cand_pos_tu avoidedT(TraceArrows &t) { return t.ta_avoid_; }
cand_pos_tu maxT(TraceArrows &t) { return t.ta_max_; }